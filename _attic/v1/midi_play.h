...
typedef
	struct {
		...
		volatile uint32_t  delta;    // Running delta
		uint32_t           uspdt;    // uS per Delta tick

		struct timeval     tmStart;  // start timestamp
		struct timeval     tmPause;  // time paused

		uint8_t            runStat;  // Running status
		uint32_t*          tDelta;   // Next track delta []
		uint8_t**          pEvent;   // Next track event []
	}
smf_t;
